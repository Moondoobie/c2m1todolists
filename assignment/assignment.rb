...
class Assignment
  ...
  def create_user(params)
      ...
      u1.password_digest = params[:password_digest]
      u1.save
  end
  ...
  def create_todolist(params)
      ...
      # return an instance of the class with primary key (`id`), and dates (`created_at` and `updated_at`) assigned
      todo = TodoList.new
      todo.list_name = params[:name]
      todo.list_due_date = params[:due_date]
      todo.save
  end
  ...
  def find_allusers(offset, limit)
     ...
     User.all.order(updated_at: :asc).offset(offset).limit(limit).to_a 
  end
  ...
  def update_password(id, password_digest)
      ...
      u1 = User.find(id)
      u1.password_digest = password_digest
      u1.save
  end
